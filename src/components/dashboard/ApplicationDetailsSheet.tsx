...
export const ApplicationDetailsSheet = ({
  ...
  useEffect(() => {
    if (selectedRow) {
      // Convert null or undefined to default values
      const flagValue = selectedRow.itr_flag === null ? "false" : String(selectedRow.itr_flag);
      const amountValue = selectedRow.lrs_amount_consumed === null ? "0" : String(selectedRow.lrs_amount_consumed);
      
      setItrFlag(flagValue);
      setLrsAmount(amountValue);
    }
